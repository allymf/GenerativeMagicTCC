...
class CanvasSKScene: SKScene {
    ...
    public func adjustTextSizeToFit() {
        
        let factor = (self.frame.size.width-32)/self.selectNode.frame.size.width
        
        let scale = factor > 1.0 ? factor-1.0 : factor

        selectNode.fontSize *= scale
